...
func init() {
	register("WAFRulePredicate", ListWAFRulePredicates)
}

func ListWAFRulePredicates(sess *session.Session) ([]Resource, error) {
	svc := waf.New(sess)
	resources := []Resource{}
	...
	for {
		...
		for _, rule := range resp.Rules {
			...
			for _, predicate := range details.Rule.Predicates {
				resources = append(resources, &WAFRulePredicate{
					svc:       svc,
					ruleID:    rule.RuleId,
					predicate: predicate,
				})
...
func (r *WAFRulePredicate) Remove() error {
...
func (r *WAFRulePredicate) Properties() types.Properties {
...
func (r *WAFRulePredicate) String() string {
